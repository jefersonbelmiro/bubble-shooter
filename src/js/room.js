;(function(global) {
    ...
    Room.prototype = {
        ...
        create: function() 
        {
            BubbleShoot.joinRoom = false;
            
            if (!BubbleShoot.server) {
                BubbleShoot.server = new BubbleShoot.Server();
            }

            BubbleShoot.server.connect(function(error) {
        ...
        createListRooms: function()
        {
            ...
            var background = BubbleShoot.game.add.sprite(10, 110, Utils.createRect(width, height, '#333'));
            ...
            var getRooms = function(error, rooms)
            {
                ...
                for (var i = 0, len = rooms.length; i < len; i++) {
                    ...
                    var bgItem = BubbleShoot.game.add.sprite(position.x, position.y, Utils.createRect(545, 48, '#222'));
            ...
            BubbleShoot.server.getRooms(getRooms.bind(this));
        },

        join: function(room)
        {
            BubbleShoot.joinRoom = room;
        ...
        back: function()
        {
            if (BubbleShoot.joinServer) {
                BubbleShoot.joinServer = false;
                BubbleShoot.server.disconnect();
            }
